ufak main ırk tanıtımı

<table class="tableRow">
    <caption>Irk Avantaj Tablosu</caption>
    <thead>
        <tr>
            <th>
                Avantaj
            </th>
            <th>
                Açıklama
            </th>
        </tr>
    </thead>
    <tbody>
        <tr>
            <td>
                ???
            </td>
            <td>
                ???
            </td>
        </tr>
    </tbody>
</table>

<table class="tableRow">
    <caption>Irk Dezavantaj Tablosu</caption>
    <thead>
        <tr>
            <th>
                Dezavantaj
            </th>
            <th>
                Açıklama
            </th>
        </tr>
    </thead>
    <tbody>
        <tr>
            <td>
                ???
            </td>
            <td>
                ???
            </td>
        </tr>
    </tbody>
</table>

## Elemental Dragonborn
<img src="worlds/test.png" width="300">
*açıklama*

<table class="tableRow">
        <caption>Özellik Tablosu</caption>
        <thead>
            <tr>
                <th>
                    Özellik Adı
                </th>
                <th>
                    Özellik
                </th>
            </tr>
        </thead>
        <tbody>
            <tr>
                <td>
                    Base HP
                </td>
                <td>
                    120
                </td>
            </tr>
            <tr>
                <td>
                    Size
                </td>
                <td>
                    Medium
                </td>
            </tr>
            <tr>
                <td>
                    Speed
                </td>
                <td>
                    8
                </td>
            </tr>
            <tr>
                <td>
                    Languages
                </td>
                <td>
                    Common, Draconic
                </td>
            </tr>
        </tbody>
</table>

<table class="tableRow">
        <caption>Yaş Tablosu</caption>
        <thead>
            <tr>
                <th>
                    Yaş
                </th>
                <th>
                    Aşama
                </th>
                <th>
                    Açıklama
                </th>
            </tr>
        </thead>
        <tbody>
            <tr>
                <td>
                    <p>0 - 2</p>
                </td>
                <td>
                    <p>Bebek</p>
                </td>
                <td>
                    <p>Fiziksel ve zihinsel gelişim başlangıcı</p>
                </td>
            </tr>
            <tr>
                <td>
                    <p>3 - 12</p>
                </td>
                <td>
                    <p>Çocuk</p>
                </td>
                <td>
                    <p>Temel öğrenme dönemi</p>
                </td>
            </tr>
            <tr>
                <td>
                    <p>13 - 19</p>
                </td>
                <td>
                    <p>Ergen</p>
                </td>
                <td>
                    <p>Fiziksel değişim, kimlik oluşumu</p>
                </td>
            </tr>
            <tr>
                <td>
                    <p>20 - 30 (20 - 1000)</p>
                </td>
                <td>
                    <p>Genç Yetişkin</p>
                </td>
                <td>
                    <p>Fiziksel zirve başlangıcı</p>
                </td>
            </tr>
            <tr>
                <td>
                    <p>31 - 50 (1001 - 2500)</p>
                </td>
                <td>
                    <p>Yetişkin</p>
                </td>
                <td>
                    <p>Dengeli dönem</p>
                </td>
            </tr>
            <tr>
                <td>
                    <p>51 - 65 (2501 - 4400)</p>
                </td>
                <td>
                    <p>Olgun</p>
                </td>
                <td>
                    <p>Tecrübe yüksek, fiziksel düşüş başlangıcı</p>
                </td>
            </tr>
            <tr>
                <td>
                    <p>66+ (4400+)</p>
                </td>
                <td>
                    <p>Yaşlı</p>
                </td>
                <td>
                    <p>Fiziksel zayıflık, zihinsel düşüş başlangıcı</p>
                </td>
            </tr>
            <tr>
                <td>
                    <p>80 (5000)</p>
                </td>
                <td>
                    <p>Ortalama Ömür</p>
                </td>
                <td>
                    <p>Ortalama ömür sonu</p>
                </td>
            </tr>
            <tr>
                <td>
                    <p>EXTRA</p>
                </td>
                <td>
                    <p>Ejderha</p>
                </td>
                <td>
                    <p>Eğer içindeki ejderha uyanmışsa parantez içleri geçerlidir.</p>
                </td>
            </tr>
        </tbody>
</table>

<table class="tableRow">
        <caption>Ahlak Tablosu</caption>
        <thead>
            <tr>
                <th> 
                </th>
                <th>
                    Lawful
                </th>
                <th>
                    Neutral
                </th>
                <th>
                    Chaotic
                </th>
            </tr>
        </thead>
        <tbody>
            <tr>
                <th>
                    Good
                </th>
                <td>
                ***
                </td>
                <td>
                **
                </td>
                <td>
                ****
                </td>
            </tr>
            <tr>
                <th>
                    Neutral
                </th>
                <td>
                *
                </td>
                <td>
                </td>
                <td>
                **
                </td>
            </tr>
            <tr>
                <th>
                    Evil
                </th>
                <td>
                **
                </td>
                <td>
                *
                </td>
                <td>
                ***
                </td>
            </tr>
        </tbody>
    </table>

<table class="tableRow">
        <caption>Önyargı Tablosu</caption>
        <thead>
            <tr>
                <th>
                    Önyargı
                </th>
                <th>
                    Irk
                </th>
            </tr>
        </thead>
        <tbody>
            <tr>
                <th>
                    Sevgi
                </th>
                <td>
                </td>
            </tr>
            <tr>
                <th>
                    İyi
                </th>
                <td>
                </td>
            </tr>
            <tr>
                <th>
                    Nötr
                </th>
                <td>
                </td>
            </tr>
            <tr>
                <th>
                    Kötü
                </th>
                <td>
                </td>
            </tr>
            <tr>
                <th>
                    Nefret
                </th>
                <td>
                </td>
            </tr>
        </tbody>
    </table>

<table class="tableRow">
        <caption>Irk Avantaj Tablosu</caption>
        <thead>
            <tr>
                <th>
                    Avantaj
                </th>
                <th>
                    Açıklama
                </th>
            </tr>
        </thead>
        <tbody>
            <tr>
                <td>
                    Draconic Ancestry (Dragon)
                </td>
                <td>
                    Bir element seçersin. Bu element'e karşı %25 resistence kazanırsın. Bu element'i ağzından üfleyebilirsin. Bazı level'larda içindeki ejderha güçlenir, ejderha güçleri listesinden güçler seçersin.
                </td>
            </tr>
            <tr>
                <td>
                    Dragon Blood
                </td>
                <td>
                    Ejder kanı bir çok kimya ve simya karışımında kullanılan değerli bir eşyadır. D Tier kan ile başlarsın.
                </td>
            </tr>
        </tbody>
</table>

<table class="tableRow">
        <caption>Irk Dezavantaj Tablosu</caption>
        <thead>
            <tr>
                <th>
                    Avantaj
                </th>
                <th>
                    Açıklama
                </th>
            </tr>
        </thead>
        <tbody>
            <tr>
                <td>
                    ???
                </td>
                <td>
                    ???
                </td>
            </tr>
        </tbody>
</table>

## Kobold
<img src="worlds/test.png" width="300">
*açıklama*

<table class="tableRow">
        <caption>Özellik Tablosu</caption>
        <thead>
            <tr>
                <th>
                    Özellik Adı
                </th>
                <th>
                    Özellik
                </th>
            </tr>
        </thead>
        <tbody>
            <tr>
                <td>
                    Base HP
                </td>
                <td>
                    90
                </td>
            </tr>
            <tr>
                <td>
                    Size
                </td>
                <td>
                    Medium
                </td>
            </tr>
            <tr>
                <td>
                    Speed
                </td>
                <td>
                    9
                </td>
            </tr>
            <tr>
                <td>
                    Languages
                </td>
                <td>
                    Common
                </td>
            </tr>
        </tbody>
</table>

<table class="tableRow">
        <caption>Yaş Tablosu</caption>
        <thead>
            <tr>
                <th>
                    Yaş
                </th>
                <th>
                    Aşama
                </th>
                <th>
                    Açıklama
                </th>
            </tr>
        </thead>
        <tbody>
            <tr>
                <td>
                    <p>0 - 2</p>
                </td>
                <td>
                    <p>Bebek</p>
                </td>
                <td>
                    <p>Fiziksel ve zihinsel gelişim başlangıcı</p>
                </td>
            </tr>
            <tr>
                <td>
                    <p>3 - 12</p>
                </td>
                <td>
                    <p>Çocuk</p>
                </td>
                <td>
                    <p>Temel öğrenme dönemi</p>
                </td>
            </tr>
            <tr>
                <td>
                    <p>13 - 19</p>
                </td>
                <td>
                    <p>Ergen</p>
                </td>
                <td>
                    <p>Fiziksel değişim, kimlik oluşumu</p>
                </td>
            </tr>
            <tr>
                <td>
                    <p>20 - 30</p>
                </td>
                <td>
                    <p>Genç Yetişkin</p>
                </td>
                <td>
                    <p>Fiziksel zirve başlangıcı</p>
                </td>
            </tr>
            <tr>
                <td>
                    <p>31 - 50</p>
                </td>
                <td>
                    <p>Yetişkin</p>
                </td>
                <td>
                    <p>Dengeli dönem</p>
                </td>
            </tr>
            <tr>
                <td>
                    <p>51 - 65</p>
                </td>
                <td>
                    <p>Olgun</p>
                </td>
                <td>
                    <p>Tecrübe yüksek, fiziksel düşüş başlangıcı</p>
                </td>
            </tr>
            <tr>
                <td>
                    <p>66+</p>
                </td>
                <td>
                    <p>Yaşlı</p>
                </td>
                <td>
                    <p>Fiziksel zayıflık, zihinsel düşüş başlangıcı</p>
                </td>
            </tr>
            <tr>
                <td>
                    <p>80</p>
                </td>
                <td>
                    <p>Ortalama Ömür</p>
                </td>
                <td>
                    <p>Ortalama ömür sonu</p>
                </td>
            </tr>
        </tbody>
</table>

<table class="tableRow">
        <caption>Ahlak Tablosu</caption>
        <thead>
            <tr>
                <th> 
                </th>
                <th>
                    Lawful
                </th>
                <th>
                    Neutral
                </th>
                <th>
                    Chaotic
                </th>
            </tr>
        </thead>
        <tbody>
            <tr>
                <th>
                    Good
                </th>
                <td>
                *
                </td>
                <td>
                **
                </td>
                <td>
                ***
                </td>
            </tr>
            <tr>
                <th>
                    Neutral
                </th>
                <td>
                **
                </td>
                <td>
                ***
                </td>
                <td>
                ****
                </td>
            </tr>
            <tr>
                <th>
                    Evil
                </th>
                <td>
                </td>
                <td>
                *
                </td>
                <td>
                **
                </td>
            </tr>
        </tbody>
    </table>

<table class="tableRow">
        <caption>Önyargı Tablosu</caption>
        <thead>
            <tr>
                <th>
                    Önyargı
                </th>
                <th>
                    Irk
                </th>
            </tr>
        </thead>
        <tbody>
            <tr>
                <th>
                    Sevgi
                </th>
                <td>
                </td>
            </tr>
            <tr>
                <th>
                    İyi
                </th>
                <td>
                </td>
            </tr>
            <tr>
                <th>
                    Nötr
                </th>
                <td>
                </td>
            </tr>
            <tr>
                <th>
                    Kötü
                </th>
                <td>
                </td>
            </tr>
            <tr>
                <th>
                    Nefret
                </th>
                <td>
                </td>
            </tr>
        </tbody>
    </table>

<table class="tableRow">
        <caption>Irk Avantaj Tablosu</caption>
        <thead>
            <tr>
                <th>
                    Avantaj
                </th>
                <th>
                    Açıklama
                </th>
            </tr>
        </thead>
        <tbody>
            <tr>
                <td>
                    Draconic Ancestry (Kobold)
                </td>
                <td>
                    Bir element seçersin. Bu element'in hasar türünü kullanarak verdiğin hasarlar +%50 daha fazla vurur. Chameleon tarzı bir görünmezlik yeteneğin olur ve concentration gerektirmediği için bozulması daha zordur. Bazı level'larda içindeki ejderha güçlenir, kobold güçleri listesinden güçler seçersin.
                </td>
            </tr>
            <tr>
                <td>
                    Dragon Blood
                </td>
                <td>
                    Ejder kanı bir çok kimya ve simya karışımında kullanılan değerli bir eşyadır. F Tier kan ile başlarsın.
                </td>
            </tr>
            <tr>
                <td>
                    Greater Dexterity
                </td>
                <td>
                    +4 DEX
                </td>
            </tr>
        </tbody>
</table>

<table class="tableRow">
        <caption>Irk Dezavantaj Tablosu</caption>
        <thead>
            <tr>
                <th>
                    Avantaj
                </th>
                <th>
                    Açıklama
                </th>
            </tr>
        </thead>
        <tbody>
            <tr>
                <td>
                    ???
                </td>
                <td>
                    ???
                </td>
            </tr>
        </tbody>
</table>

## Wyrmblood
<img src="worlds/test.png" width="300">
*açıklama*

<table class="tableRow">
        <caption>Özellik Tablosu</caption>
        <thead>
            <tr>
                <th>
                    Özellik Adı
                </th>
                <th>
                    Özellik
                </th>
            </tr>
        </thead>
        <tbody>
            <tr>
                <td>
                    Base HP
                </td>
                <td>
                    200
                </td>
            </tr>
            <tr>
                <td>
                    Size
                </td>
                <td>
                    Medium
                </td>
            </tr>
            <tr>
                <td>
                    Speed
                </td>
                <td>
                    7
                </td>
            </tr>
            <tr>
                <td>
                    Languages
                </td>
                <td>
                    Common, Draconic
                </td>
            </tr>
        </tbody>
</table>

<table class="tableRow">
        <caption>Yaş Tablosu</caption>
        <thead>
            <tr>
                <th>
                    Yaş
                </th>
                <th>
                    Aşama
                </th>
                <th>
                    Açıklama
                </th>
            </tr>
        </thead>
        <tbody>
            <tr>
                <td>
                    <p>0 - 2</p>
                </td>
                <td>
                    <p>Bebek</p>
                </td>
                <td>
                    <p>Fiziksel ve zihinsel gelişim başlangıcı</p>
                </td>
            </tr>
            <tr>
                <td>
                    <p>3 - 12</p>
                </td>
                <td>
                    <p>Çocuk</p>
                </td>
                <td>
                    <p>Temel öğrenme dönemi</p>
                </td>
            </tr>
            <tr>
                <td>
                    <p>13 - 19</p>
                </td>
                <td>
                    <p>Ergen</p>
                </td>
                <td>
                    <p>Fiziksel değişim, kimlik oluşumu</p>
                </td>
            </tr>
            <tr>
                <td>
                    <p>20 - 30 (20 - 1000)</p>
                </td>
                <td>
                    <p>Genç Yetişkin</p>
                </td>
                <td>
                    <p>Fiziksel zirve başlangıcı</p>
                </td>
            </tr>
            <tr>
                <td>
                    <p>31 - 50 (1001 - 2500)</p>
                </td>
                <td>
                    <p>Yetişkin</p>
                </td>
                <td>
                    <p>Dengeli dönem</p>
                </td>
            </tr>
            <tr>
                <td>
                    <p>51 - 65 (2501 - 4400)</p>
                </td>
                <td>
                    <p>Olgun</p>
                </td>
                <td>
                    <p>Tecrübe yüksek, fiziksel düşüş başlangıcı</p>
                </td>
            </tr>
            <tr>
                <td>
                    <p>66+ (4400+)</p>
                </td>
                <td>
                    <p>Yaşlı</p>
                </td>
                <td>
                    <p>Fiziksel zayıflık, zihinsel düşüş başlangıcı</p>
                </td>
            </tr>
            <tr>
                <td>
                    <p>80 (5000)</p>
                </td>
                <td>
                    <p>Ortalama Ömür</p>
                </td>
                <td>
                    <p>Ortalama ömür sonu</p>
                </td>
            </tr>
            <tr>
                <td>
                    <p>EXTRA</p>
                </td>
                <td>
                    <p>Ejderha</p>
                </td>
                <td>
                    <p>Eğer içindeki ejderha uyanmışsa parantez içleri geçerlidir.</p>
                </td>
            </tr>
        </tbody>
</table>

<table class="tableRow">
        <caption>Ahlak Tablosu</caption>
        <thead>
            <tr>
                <th> 
                </th>
                <th>
                    Lawful
                </th>
                <th>
                    Neutral
                </th>
                <th>
                    Chaotic
                </th>
            </tr>
        </thead>
        <tbody>
            <tr>
                <th>
                    Good
                </th>
                <td>
                ****
                </td>
                <td>
                **
                </td>
                <td>
                ***
                </td>
            </tr>
            <tr>
                <th>
                    Neutral
                </th>
                <td>
                **
                </td>
                <td>
                </td>
                <td>
                *
                </td>
            </tr>
            <tr>
                <th>
                    Evil
                </th>
                <td>
                ***
                </td>
                <td>
                *
                </td>
                <td>
                **
                </td>
            </tr>
        </tbody>
    </table>

<table class="tableRow">
        <caption>Önyargı Tablosu</caption>
        <thead>
            <tr>
                <th>
                    Önyargı
                </th>
                <th>
                    Irk
                </th>
            </tr>
        </thead>
        <tbody>
            <tr>
                <th>
                    Sevgi
                </th>
                <td>
                </td>
            </tr>
            <tr>
                <th>
                    İyi
                </th>
                <td>
                </td>
            </tr>
            <tr>
                <th>
                    Nötr
                </th>
                <td>
                </td>
            </tr>
            <tr>
                <th>
                    Kötü
                </th>
                <td>
                </td>
            </tr>
            <tr>
                <th>
                    Nefret
                </th>
                <td>
                </td>
            </tr>
        </tbody>
    </table>

<table class="tableRow">
        <caption>Irk Avantaj Tablosu</caption>
        <thead>
            <tr>
                <th>
                    Avantaj
                </th>
                <th>
                    Açıklama
                </th>
            </tr>
        </thead>
        <tbody>
            <tr>
                <td>
                    Draconic Ancestry (Wyvern)
                </td>
                <td>
                    Bir element seçersin. Bu element'e karşı %100 resistence kazanırsın. Bazı level'larda içindeki ejderha güçlenir, wyvern güçleri listesinden güçler seçersin.
                </td>
            </tr>
            <tr>
                <td>
                    Dragon Blood
                </td>
                <td>
                    Ejder kanı bir çok kimya ve simya karışımında kullanılan değerli bir eşyadır. C Tier kan ile başlarsın.
                </td>
            </tr>
            <tr>
                <td>
                    Hard Skin
                </td>
                <td>
                    +2 STR , +2 CON
                </td>
            </tr>
        </tbody>
</table>

<table class="tableRow">
        <caption>Irk Dezavantaj Tablosu</caption>
        <thead>
            <tr>
                <th>
                    Avantaj
                </th>
                <th>
                    Açıklama
                </th>
            </tr>
        </thead>
        <tbody>
            <tr>
                <td>
                    ???
                </td>
                <td>
                    ???
                </td>
            </tr>
        </tbody>
</table>
